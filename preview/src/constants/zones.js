...
// A production build is served by the dashboard reverse proxy, so the safest
// default is the page's own origin. This avoids baking a Render hostname into
// the JavaScript bundle and works with preview/custom domains too. Tests and
// server-side tooling have no window, so they retain the local supervisor URL.
function defaultWsUrl() {
  if (typeof window === 'undefined' || !window.location?.host) return 'ws://localhost:8080/ws'
